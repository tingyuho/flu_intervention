...
from fitting_model_function import *
import numpy as np
import sys
from function_flu import *
...
# step 0 intialization        
    ...
    l_partition = [0 for _ in range(i_dim)]
...
# step 5: Maintain, prune, and branch
            ...
            if b_stopping_branchable == True: # (1)
                ...
                i_partition_index = l_partition.index(min(l_partition))
                l_partition[i_partition_index] += 1
                for i in (i for i in l_subr if i.s_label == 'C' and i.b_activate == True and i.b_branchable == True):
                     ...
                     l_temp_new_branching_subr += fun_reg_branching(i, i_dim, i_B, i_partition_index)
    ...
    for c in l_subr:
        n_total_sample += c.pd_sample_record.shape[0]
    print('Final total number of sampleings: ' + str(n_total_sample))
    print ('[f_CI_u,f_CI_l]: '+str([f_CI_u,f_CI_l]))
