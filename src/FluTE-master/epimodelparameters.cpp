...
EpiModelParameters::EpiModelParameters(const char *configname) {
  ...
  nReimbursement = 0.0;// added by TingYu
  fCostSharingRate = 0.0; // added by TingYu
/*
  // load hospitalization amd death prbability add TingYu
  hosipalization_pr[TAG] = {.0141, 0.0006, 0.0042, 0.0193, 0.0421} ; // age-specific hospitalization probability
  dead_pr[TAG] = { 0.00004, 0.00001, 0.00009, 0.00134, 0.0117} ; // age-specific death probability

// Load all the cost add TingYu

  not_medical_direct_cost[TAG] = {3, 3, 3, 3, 3};
  not_medical_indirect_cost[TAG] = {145, 72.5, 72.5, 72.5, 145};

  hospitalization_highRisk_direct_cost[TAG] = {81596, 41918, 47722, 41309, 16750};
  hospitalization_nonHighRisk_direct_cost[TAG] = {10880, 15014, 19012, 22304, 11451};
  hospitalization_highRisk_indirect_cost[TAG] = {4495, 3335, 3045, 3480, 2610};
  hospitalization_nonHighRisk_indirect_cost[TAG] = {1160, 1305, 1740, 1885, 1885};

  death_highRisk_cost[TAG] = {267954, 267954, 75890, 118842, 33011};
  death_nonHighRisk_cost[TAG] = {28818, 28818, 76336, 118575, 41948};

  antivirus_highRisk_direct_cost[TAG] = {574, 649, 725, 733, 476};
  antivirus_nonHighRisk_direct_cost[TAG] = {167, 95, 125, 150, 242};
  antivirus_highRisk_indirect_cost[TAG] = {870, 580, 290, 580, 1015};
  antivirus_nonHighRisk_indirect_cost[TAG] = {145, 145, 145, 290, 435};

  vaccination_direct_cost[TAG] = {65, 65, 65, 65, 65};
  vaccination_indirect_cost[TAG] = {0, 0, 0, 0, 0};
*/
...
bool EpiModelParameters::readConfigFile(const char *configname) {
  ...
  if (!iss) {
    ...
    } else if (param.compare("EFFs")==0) {
	    read_config_double(EFFs, iss, "EFFs", 0.0, 1.0);
	...
	} else if (param.compare("reimbursement")==0) { //added by TingYu
	    read_config_double(nReimbursement, iss, "amount of reimbursement", 0.0, 100.0);
	} else if (param.compare("costsharingrate")==0) { //added by TingYu
	    read_config_double(fCostSharingRate, iss, "cost-sharing rate", 0.0, 1.0);
