...
#include <sstream> // added by TingYu
#include <string> // added by TingYu
#include <iomanip> // added by TingYu
#ifdef PARALLEL
  #include <mpi.h>
#endif
extern "C" {
...
EpiModel::EpiModel(EpiModelParameters &params) {
  ...
  // TingYu input experiments
  nReimbursement=params.getReimbursement();
  fCostSharingRate=params.getCostSharingRate();
  // epidemic parameters
  ...
  // added by TingYu
  memcpy(not_medical_direct_cost, params.getnot_medical_direct_cost(), TAG*sizeof(double));
  memcpy(not_medical_indirect_cost, params.getnot_medical_indirect_cost(), TAG*sizeof(double));
  memcpy(hospitalization_highRisk_direct_cost, params.gethospitalization_highRisk_direct_cost(), TAG*sizeof(double));
  memcpy(hospitalization_nonHighRisk_direct_cost, params.gethospitalization_nonHighRisk_direct_cost(), TAG*sizeof(double));
  memcpy(hospitalization_highRisk_indirect_cost, params.gethospitalization_highRisk_indirect_cost(), TAG*sizeof(double));
  memcpy(hospitalization_nonHighRisk_indirect_cost, params.gethospitalization_nonHighRisk_indirect_cost(), TAG*sizeof(double));
  memcpy(death_highRisk_cost, params.getdeath_highRisk_cost(), TAG*sizeof(double));
  memcpy(death_nonHighRisk_cost, params.getdeath_nonHighRisk_cost(), TAG*sizeof(double));
  memcpy(antivirus_highRisk_direct_cost, params.getantivirus_highRisk_direct_cost(), TAG*sizeof(double));
  memcpy(antivirus_nonHighRisk_direct_cost, params.getantivirus_nonHighRisk_direct_cost(), TAG*sizeof(double));
  memcpy(antivirus_highRisk_indirect_cost, params.getantivirus_highRisk_indirect_cost(), TAG*sizeof(double));
  memcpy(antivirus_nonHighRisk_indirect_cost, params.getantivirus_nonHighRisk_indirect_cost(), TAG*sizeof(double));
  memcpy(vaccination_direct_cost, params.getvaccination_direct_cost(), TAG*sizeof(double));
  memcpy(vaccination_indirect_cost, params.getvaccination_indirect_cost(), TAG*sizeof(double));
...
void EpiModel::create_person(int nAgeGroup, int nFamilySize, int nFamily, int nHouseholdCluster, int nNeighborhood, int nSchoolgroup, Community& comm) {
  ...
  p.sourceid = p.id;
  p.nInfectedTime = -1;
  p.everinfected = 0; //added by TingYu
  p.everantivirus = 0; //added by TingYu
  p.nVaccinePriority=0;
  ...
  if (fPregnantFraction[p.age]>0.0 && get_rand_double<fPregnantFraction[p.age])
    setPregnant(p);
  // New data added by TingYu
  //begin:
  p.expected_cost_2_stage = 0; // expected cost when determining taking antivirus or not
  if (p.age == 0){
    p.prob_hospitalization = 0.0020266;  // probability of hospitalization add TingYu
    p.prob_death = 0.0000057; // probability of death add TingYu
  }
  if (p.age == 1){
    p.prob_hospitalization = 0.0000857;  // probability of hospitalization add TingYu
    p.prob_death = 0.0000014; // probability of death add TingYu
  }
   if (p.age == 2){
    p.prob_hospitalization = 0.0006011;  // probability of hospitalization add TingYu
    p.prob_death = 0.0000129; // probability of death add TingYu
  }
   if (p.age == 3){
    p.prob_hospitalization = 0.0027802;  // probability of hospitalization add TingYu
    p.prob_death = 0.0001915; // probability of death add TingYu
  }
   if (p.age == 4){
    p.prob_hospitalization = 0.0061257;  // probability of hospitalization add TingYu
    p.prob_death = 0.0016799; // probability of death add TingYu
  }
  //p.prob_hospitalization = 0;  // probability of hospitalization add TingYu
  ...
  if (isHighRisk(p)){
    ...
    if (p.age ==0){
	  ...
	  p.direct_cost_hospitalization = 81596.0;
	  p.indirect_cost_hospitalization = 4495.0;
    ...
    if (p.age ==1){
      p.direct_cost_not_medical = 3.0;
      p.indirect_cost_not_medical = 72.5;
	  p.direct_cost_hospitalization = 41918.0;
	  p.indirect_cost_hospitalization = 3335.0;
	  p.cost_death = 267954.0;
    ...
    if (p.age ==2){
      p.direct_cost_not_medical = 3.0;
      p.indirect_cost_not_medical = 72.5;
	  p.direct_cost_hospitalization = 47722.0;
	  p.indirect_cost_hospitalization = 3045.0;
    ...
    if (p.age ==3){
      ...
      p.indirect_cost_not_medical = 72.5 ;
	  p.direct_cost_hospitalization = 41309;
	  p.indirect_cost_hospitalization = 3480;
	  p.cost_death = 118842;
    ...
    if (p.age ==4){
	  ...
	  p.direct_cost_hospitalization = 16750;
	  p.indirect_cost_hospitalization = 2610;
	  p.cost_death = 33011;
  ...
  if (!isHighRisk(p)){
    ...
    if (p.age ==0){
      p.direct_cost_not_medical = 3;
	  p.indirect_cost_not_medical = 145;
	  p.direct_cost_hospitalization = 10880;
	  p.indirect_cost_hospitalization = 1160;
	  p.cost_death = 28818;
    ...
    if (p.age ==1){
      p.direct_cost_not_medical = 3;
	  p.indirect_cost_not_medical = 72.5;
	  p.direct_cost_hospitalization = 15014;
	  p.indirect_cost_hospitalization = 1305;
	  p.cost_death = 28818;
    ...
    if (p.age ==2){
      p.direct_cost_not_medical = 3;
	  p.indirect_cost_not_medical = 72.5;
	  p.direct_cost_hospitalization = 19012;
	  p.indirect_cost_hospitalization = 1740;
	  p.cost_death = 76336;
    ...
    if (p.age ==3){
      p.direct_cost_not_medical = 3;
	  p.indirect_cost_not_medical = 72.5;
	  p.direct_cost_hospitalization = 22304;
	  p.indirect_cost_hospitalization = 1885;
	  p.cost_death = 118575;
    ...
    if (p.age ==4){
      p.direct_cost_not_medical = 3.0;
	  p.indirect_cost_not_medical = 145.0;
	  p.direct_cost_hospitalization = 11451.0;
	  p.indirect_cost_hospitalization = 1885.0;
...
/*
  if (isHighRisk(p)){
	p.direct_cost_not_medical = not_medical_direct_cost[p.age];
	p.indirect_cost_not_medical = not_medical_indirect_cost[p.age] ;
	p.direct_cost_hospitalization = hospitalization_highRisk_direct_cost[p.age];
	p.indirect_cost_hospitalization = hospitalization_highRisk_indirect_cost[p.age];
	p.cost_death = death_highRisk_cost[p.age];
	p.direct_cost_antivirus = antivirus_highRisk_direct_cost[p.age];
	p.indirect_cost_antivirus = antivirus_highRisk_indirect_cost[p.age];
	p.indirect_cost_vaccination = vaccination_indirect_cost[p.age];
  }
  if (!isHighRisk(p)){
	p.direct_cost_not_medical = not_medical_direct_cost[p.age];
	p.indirect_cost_not_medical = not_medical_indirect_cost[p.age] ;
	p.direct_cost_hospitalization = hospitalization_nonHighRisk_direct_cost[p.age];
	p.indirect_cost_hospitalization = hospitalization_nonHighRisk_indirect_cost[p.age];
	p.cost_death = death_highRisk_cost[p.age];
	p.direct_cost_antivirus = antivirus_nonHighRisk_direct_cost[p.age];
	p.indirect_cost_antivirus = antivirus_nonHighRisk_indirect_cost[p.age];
	p.indirect_cost_vaccination = vaccination_indirect_cost[p.age];
  }
  */
  p.cost_unviccinated = 0; // cost of unvaccination add TingYu
  p.cost_viccinated = 0; // cost of unvaccination add TingYu
  p.cost_antivirus = 0; // cost of antivirus add TingYu
  p.cost_notantivirus = 0; // cost of not taking antivirus add TingYu
  //end
#ifdef PARALLEL
...
void EpiModel::infect(Person& p) {
  ...
  p.everinfected = 1; //added by TingYu
  ...
  if (isVaccinated(p)) {
    ...
    if (needsBoost(p))
      ...
      */
        fSymptomaticProb*=(1.0-defaultvacceff[p.vday]*fVaccineEfficacyByAge[p.age]); //added by TingYu
  }
  if (isAntiviral(p))
...
void EpiModel::day(void) {
  ...
  for (vector< Person >::iterator it = pvec.begin();
    ...
    */
    p.prs *= (1.0-defaultvacceff[p.vday]*fVaccineEfficacyByAge[p.age]); //added by TingYu
	p.pri *= (1.0-defaultvacceff[p.vday]*fVaccineEfficacyByAge[p.age]); //added by TingYu
    }
...
/*
 * hopitalized add by TingYu
 */
 void EpiModel::hopitalized(Person& p) {
	setHospitalized(p);
}
/*
 * dead add by TingYu
 */
 void EpiModel::dead(Person& p) {
	setDead(p);
}
/* absorbing add by TingYu
 * Determine if the person is hospitalized or dead
 */
 void EpiModel::absorbing(void) {
	vector< Person >::iterator pend = pvec.end();
	for (vector< Person >::iterator it = pvec.begin();
		it != pend;
		it++) {
		Person &p = *it;
		if (isInfected(p) && !isHospitalized(p) && !isDead(p) && !isAntiviral(p) && !isVaccinated(p)){
			if (get_rand_double < p.prob_hospitalization)
				hopitalized(p);
			if (get_rand_double < p.prob_death)
				dead(p);
		}
		if (isInfected(p) && !isHospitalized(p) && !isDead(p) && !isAntiviral(p) && isVaccinated(p)){
			if (get_rand_double < p.prob_hospitalization*(1-0.4*fVaccineEfficacyByAge[p.age])) // if efficacy is one, then reduce 40% of hospitalzation and death
				hopitalized(p);
			if (get_rand_double < p.prob_death*(1-0.4*fVaccineEfficacyByAge[p.age]))
				dead(p);
		}
    }
}
/* vaccination decision making add by TingYu
 *
 */
 void EpiModel::decisionMaking_vaccination(void) {
	...
	for (vector< Person >::iterator it = pvec.begin();
	   ...
	   it++) {
            Person &p = *it;
            // TingYu: add decision making of vaccination or not
            if (!isVaccinated(p) && !isAntiviral(p) && isSusceptible(p) && p.everantivirus == 0 && p.everinfected == 0){
                p.cost_notantivirus = (1-p.prob_hospitalization-p.prob_death)*(p.direct_cost_not_medical+p.indirect_cost_not_medical)+p.prob_hospitalization*(fCostSharingRate*p.direct_cost_hospitalization+p.indirect_cost_hospitalization)+p.prob_death*(fCostSharingRate*p.cost_death);
                ...
                if (p.cost_notantivirus >= p.cost_antivirus)
                    {p.expected_cost_2_stage = p.cost_antivirus;}
                if (p.cost_notantivirus <= p.cost_antivirus)
                    {p.expected_cost_2_stage = p.cost_notantivirus;}
                //begin: calculate prob_vac and prob_unv
                for (vector< Community >::iterator it = commvec.begin();
                       it != cend;
                       it++) {
                    Community &comm = *it;
                    if ((comm.id == p.nDayComm) && (comm.ninf[0]>0 || comm.ninf[1]>0 || comm.ninf[2]>0 || comm.ninf[3]>0 || comm.ninf[4]>0)) { // is any resident currently infected?
                        for (unsigned int pid=comm.nFirstPerson; pid<comm.nLastPerson; pid++) {
                            Person &p2 = pvec[pid];
                            if (isInfectious(p2) && !isWithdrawn(p2) && !isQuarantined(p2) && p2.nDayComm==comm.id && p2.nTravelTimer<=0) {
                                const double *cpf = (isChild(p2)?cpfc:cpfa); // probability of family transmission
                                if (p2.family == p.family){
                                    p.prob_unv *= (1-cpf[p2.age]*p.prs*p2.pri);
                                    p.prob_vac *= (1-cpf[p2.age]*(1-EFFs)*p.prs*p2.pri);  // 1-x: x is efficacy
                                    }
                                if (p2.nDayNeighborhood == p.nDayNeighborhood){
                                    int casualmultiplier2 = 2;
                                    p.prob_unv *= (1-comm.daycpnh[p2.age]*casualmultiplier2*p.prs*p2.pri);
                                    p.prob_vac *= (1-comm.daycpnh[p2.age]*casualmultiplier2*(1-EFFs)*p.prs*p2.pri);
                                    }
                                if (isChild(p2) && isChild(p) && p2.nWorkplace==p.nWorkplace) {  // transmitter is child
                                    p.prob_unv *= (1-(p2.nWorkplace>=9?cps[9]:comm.cps[p2.nWorkplace])*p.prs*p2.pri);
                                    p.prob_vac *= (1-(p2.nWorkplace>=9?cps[9]:comm.cps[p2.nWorkplace])*(1-EFFs)*p.prs*p2.pri);  // 1-x: x is efficacy
                                     // transmission within school/daycare/playgroup
                                    }
                                else {           // transmitter is adult
                                    if (isWorkingAge(p) && p2.nWorkplace==p.nWorkplace){
                                        p.prob_unv *= (1-cpw*p.prs*p2.pri);
                                        p.prob_vac *= (1-cpw*(1-EFFs)*p.prs*p2.pri); // 1-x: x is efficacy
                ...
                p.prob_unv = 1-p.prob_unv;
                p.prob_vac = 1-p.prob_vac;
                //end: calculate prob_vac and prob_unv
                p.cost_unviccinated = p.prob_unv*p.expected_cost_2_stage;
                p.cost_viccinated = (1-p.prob_vac)*(p.indirect_cost_vaccination)+p.prob_vac*(p.indirect_cost_vaccination+p.expected_cost_2_stage)-nReimbursement;
                if (p.cost_unviccinated >= p.cost_viccinated){
                    //p.bWantVac = true;
                    //nNumWantVaccine++;
                    setVaccinated(p);
                    p.vday = 0;
 ...
 /* antivirus decision making add by TingYu
 *
 */
...
void EpiModel::response(void) {
  ...
  if (bTrigger && nTimer==nTriggerTime) {
	...
	if (!isSchoolClosed(t,1)) {
     ...
     // if (eVaccinationStrategy==MASSVAC && !isVaccinated(t)) //TingYu deactivate
	//vaccinate(t); //TingYu deactivate
    }
  }
...
/*
 * Create TingYu final report 'TingYu_summary'
 */
 void EpiModel::cost_summary(void) {

  unsigned int nDeath; // number of dead people added by TingYu
  unsigned int nHospitalized; // number of hopitalized people added by TingYu
  unsigned int nVaccination; // number of vaccinated people added by TingYu
  unsigned int nAntivirus; // number of people getting antivirus added by TingYu
  double costDeath; // cost of death spent by insurer added by TingYu
  double costHospitalized;// cost of hospitalization spent by insurer added by TingYu
  double costVaccinated;// cost of vaccination spent by insurer added by TingYu
  double costAntivirus;// cost of vaccination spent by insurer added by TingYu
  double costTotal; // total insurers cost

  costDeath = costHospitalized = costVaccinated = costAntivirus = 0;
  nDeath = nHospitalized = nVaccination = nAntivirus = 0;
  for (vector< Person >::iterator it = pvec.begin(); // calulate insurers total added by TingYu
	  it != pvec.end();
	  it++) {
	Person &p = *it;
	if (isHospitalized(p)){
	  nHospitalized=nHospitalized+1;
	  costHospitalized = costHospitalized + p.direct_cost_hospitalization * (1.0-fCostSharingRate);
	  }
	if (isDead(p)){
	  nDeath = nDeath+1;
	  costDeath = costDeath + p.cost_death * (1-fCostSharingRate);
	  }
	if (isVaccinated(p)){
	  nVaccination = nVaccination+1;
	  costVaccinated = costVaccinated + nReimbursement;
	  }
	if (isAntiviral(p)){
	  nAntivirus = nAntivirus+1;
	  costAntivirus += p.direct_cost_antivirus * (1-fCostSharingRate);
	}
  }
  costTotal = costDeath + costHospitalized + costVaccinated + costAntivirus;

	ifstream checkfile("insurer_cost_summary.csv");
	if (!checkfile.good()){
	   ofstream output_file;
	   output_file.open ("insurer_cost_summary.csv");
	   output_file << "reimbursement,cost_sharing,N_vaccination,N_antivirus,N_hospitalized,N_death,cost_Death,cost_Hospitalized,cost_Vaccinated,cost_Antivirus,total_intervention_cost,Cumulated_Infection\n";
	   output_file << nReimbursement << ",";
	   output_file << fCostSharingRate << ",";
	   output_file << nVaccination << ",";
       for (vector< unsigned int >::iterator it = nNumAntiviralsUsedHistory.begin();
         it != nNumAntiviralsUsedHistory.end();
         it++){
         if (it == nNumAntiviralsUsedHistory.end()-1) output_file << *it << ",";
       }
	   output_file << nHospitalized << ",";
	   output_file << nDeath << ",";
	   output_file << costDeath << ",";
	   output_file << costHospitalized << ",";
	   output_file << costVaccinated << ",";
	   output_file << costAntivirus << ",";
	   output_file << costTotal << ",";
	   for (vector< unsigned int >::iterator it = nNumCumulativeSymptomaticHistory.begin();
        it != nNumCumulativeSymptomaticHistory.end();
        it++){
        if (it == nNumCumulativeSymptomaticHistory.end()-1) output_file << *it << "\n";
        }
	   output_file.close();
	   //return 0;
	}
	else{
	  ofstream  output_file("insurer_cost_summary.csv", std::ofstream::out | std::ofstream::app);
	  output_file << nReimbursement << ",";
	  output_file << fCostSharingRate << ",";
      output_file << nVaccination << ",";
      output_file << nAntivirus << ",";
      output_file << nHospitalized << ",";
      output_file << nDeath << ",";
      output_file << costDeath << ",";
      output_file << costHospitalized << ",";
...
void EpiModel::summary(void) {
  ...
  /*
  unsigned int nDeath[TAG]; // number of dead people added by TingYu
  unsigned int nHospitalized[TAG]; // number of hopitalized people added by TingYu
  unsigned int nVaccination[TAG]; // number of vaccinated people added by TingYu
  unsigned int nAntivirus[TAG]; // number of people getting antivirus added by TingYu
  double costDeath; // cost of death spent by insurer added by TingYu
  double costHospitalized;// cost of hospitalization spent by insurer added by TingYu
  double costVaccinated;// cost of vaccination spent by insurer added by TingYu
  double costAntivirus;// cost of vaccination spent by insurer added by TingYu
  double costTotal; // total insurers cost
  costDeath = costHospitalized = costVaccinated = costAntivirus = 0;
  for (int j=0; j<TAG; j++) // initialize added by TingYu
	nDeath[j] = nHospitalized[j] = nVaccination[j] = nAntivirus[j] = 0;
  for (vector< Person >::iterator it = pvec.begin(); // calulate insurers total added by TingYu
	  it != pvec.end();
	  it++) {
	Person &p = *it;
	if (isHospitalized(p))
	  nHospitalized[p.age]++;
	  costHospitalized += p.direct_cost_hospitalization * (1-fCostSharingRate);
	if (isDead(p))
	  nDeath[p.age]++;
	  costDeath += p.cost_death * (1-fCostSharingRate);
	if (isVaccinated(p))
	  nVaccination[p.age]++;
	  costVaccinated += nReimbursement;
	if (isAntiviral(p))
	  nAntivirus[p.age]++;
      costAntivirus += p.direct_cost_antivirus * (1-fCostSharingRate);
  }
...
void EpiModel::run(void) {
  ...
  while(nTimer<nRunLength*2) {
    ...
    response();
    decisionMaking_vaccination() ; // TingYu add: individual making decision
	decisionMaking_antivirus() ; // TingYu add: individual making decision
	absorbing(); // TingYu add: individual could go to hospital or dead
