...
enum {
  ...
  HOSPITALIZED  = 0x80u,  // added by TingYu
  DEAD = 0*100u // added by TingYu
...
struct Person {
  ...
  // absorbing state information add TingYu
  double prob_hospitalization; //probability of hospitalization
  double prob_death; //probability of death
  double expected_cost_2_stage; // expected cost when determining taking antivirus or not, used in the first stage
  double prob_unv; // probability of getting infection when not vaccinated
  double prob_vac; // probability of getting infection when vaccinated

  // parameter added by TingYu
  double direct_cost_not_medical;
  double indirect_cost_not_medical;
  double direct_cost_hospitalization;
  double indirect_cost_hospitalization;
  double cost_death;
  double direct_cost_antivirus;
  double indirect_cost_antivirus;
  double indirect_cost_vaccination;

  // cost add TingYu
  double cost_unviccinated;
  double cost_viccinated;
  double cost_antivirus;
  double cost_notantivirus;

  // ever add TingYu
  int everinfected;
  int everantivirus;
  /*
  double direct_cost_not_medical[TAG];
  double indirect_cost_not_medical[TAG];
  double direct_cost_hospitalization[TAG];
  double indirect_cost_hospitalization[TAG];
  double cost_death[TAG];
  double direct_cost_antivirus[TAG];
  double indirect_cost_antivirus[TAG];
  double indirect_cost_vaccination[TAG];
  */
  // temporary data
  ...
  // TingYu add: create new status: hopsitalized and dead
  friend inline bool isHospitalized(const Person &p) { return p.status&HOSPITALIZED; }
  friend inline void setHospitalized(Person &p) {p.status|=HOSPITALIZED;}
  friend inline bool isDead(const Person &p) { return p.status&DEAD; }
  friend inline void setDead(Person &p) {p.status|=DEAD;}
...
class EpiModel {
 ...
 protected:
  ...
  //added by TingYu
  //begin
  void hopitalized(Person& p);
  void dead(Person& p);
  void absorbing(void);
  void decisionMaking_vaccination(void);
  void decisionMaking_antivirus(void);
  virtual void cost_summary(void);
  //end
#ifdef PARALLEL
  void sync(void);
#endif
 ...
 protected:
  ...
  // TingYu parameters
  double nReimbursement; // amount of reimbursement
  double fCostSharingRate; // proportion of cost-sharing rate

  double not_medical_direct_cost[TAG];
  double not_medical_indirect_cost[TAG];

  double hospitalization_highRisk_direct_cost[TAG];
  double hospitalization_nonHighRisk_direct_cost[TAG];
  double hospitalization_highRisk_indirect_cost[TAG];
  double hospitalization_nonHighRisk_indirect_cost[TAG];

  double death_highRisk_cost[TAG];
  double death_nonHighRisk_cost[TAG];

  double antivirus_highRisk_direct_cost[TAG];
  double antivirus_nonHighRisk_direct_cost[TAG];
  double antivirus_highRisk_indirect_cost[TAG];
  double antivirus_nonHighRisk_indirect_cost[TAG];

  double vaccination_direct_cost[TAG];
  double vaccination_indirect_cost[TAG];
  ...
  double EFFs;   // added by TingYu efficacy
