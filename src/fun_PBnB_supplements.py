...
def saveData(l_subr):
    ...
    data_file_name = 'PBnB_samplings.csv'

    _DIREPATH_ = os.getcwd()
    _RESULT_ = os.path.join(_DIREPATH_, 'results')
    _RESULT_FILE_DIR_ = os.path.join(_RESULT_, data_file_name)

    dim = 2
    l_coordinate_sample = []
    for i in range(0, dim):  # sample coordinate
        l_coordinate_sample.append("x" + str(i + 1))
        pd_samples_record = pd.DataFrame([], columns=[])
    for i in range(0, dim):  # create the dataset record the sampling data corordinate in this subregion
        pd_samples_record[l_coordinate_sample[i]] = pd.Series(float, index=pd_samples_record.index)
        pd_samples_record['# rep'] = pd.Series(0, index=pd_samples_record.index)  # number of replication
        pd_samples_record['# rep'].astype(int)
        pd_samples_record['mean'] = pd.Series(0, index=pd_samples_record.index)
        pd_samples_record['var'] = pd.Series(0, index=pd_samples_record.index)
        pd_samples_record['SST'] = pd.Series(0, index=pd_samples_record.index)
    for i in (i for i in l_subr if i.b_activate == True):
        pd_samples_record = pd.concat([pd_samples_record, i.pd_sample_record])
    pd_samples_record.to_csv(_RESULT_FILE_DIR_, index=False)
...
def fun_reg_branching(c_subr, i_dim, i_B, i_partition_index):
    import numpy as np
    import copy
